...
module type dpsolve = {
  ...
  type param =
    { sa_max: i64
    , sa_min: -- Maximum and minimum numbers of
              i64
    , sa_tol: --   successive approximation steps.
              t
    , max_fxpiter: -- Stopping tolerance for successive
                   --   approximation.
                   i64
    , pi_max: -- Maximum number of times to switch
              --   between Newton-Kantorovich and
              --   successive approximation iterations.
              i64
    , pi_tol: -- Maximum number of Newton-Kantorovich steps.
              t
    , tol_ratio: -- Final exit tolerance in fixed point
                 --   algorithm, measured in units of
                 --   numerical precision
                 t
    }

  -- Relative tolerance before switching
...
module mk_dpsolve
  ...
  type param =
    { sa_max: i64
    , sa_min: -- Maximum number of contraction steps
              i64
    , sa_tol: -- Minimum number of contraction steps
              t
    , max_fxpiter: -- Absolute tolerance before (in dpsolve.poly: tolerance before switching
                   --   to N-K algorithm)
                   i64
    , pi_max: -- Maximum number of times to switch between Newton-Kantorovich iterations
              --   and contraction iterations.
              i64
    , pi_tol: -- Maximum number of Newton-Kantorovich steps
              t
    , tol_ratio: -- Final exit tolerance in fixed point algorithm, measured in units of
                 --   numerical precision
                 t
    }

  -- Relative tolerance before switching to N-K algorithm
  ...
  def nk [m]
    ...
    while !conv && i < ap.pi_max do
      ...
      let V = map2 (T.-) V0 (ols_jac.ols F (map2 (T.-) V0 V1))
      -- NK-iteration
      ...
      let ltol = T.(ap.pi_tol * (i64 10 ** adj))
      -- Adjust final tolerance
